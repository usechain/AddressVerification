...
import (
	...
	"github.com/usechain/go-usechain/commitee/verifier"
...
//For testing E(m)
		mainAccountPub := GetMainAccountPub()
		shareInt := big.NewInt(1)
		shareData := CountSubAccountSharePart(shareInt, mainAccountPub)

		A1 := new(ecdsa.PublicKey)
		A1.Curve = crypto.S256()
		A1.X, A1.Y = crypto.S256().Add(shareData.X, shareData.Y, polynomialInt[0].X, polynomialInt[0].Y)
*/
		verifier.GenerateSubAccount(&polynomialInt[0], SPub)
		...
		//pubSum.X, pubSum.Y = crypto.S256().Add(pubSum.X, pubSum.Y, polynomialInt[0].X, polynomialInt[0].Y)

/*
		fmt.Printf("%x, %x\n", pubSum.X, pubSum.Y)
		fmt.Printf("%x, %x\n", polynomialInt[0].X, polynomialInt[0].Y)

		var sharesTest []string = make([]string, 2)
		sharesTest[0] = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=" + toBase64(pubSum.X)
		sharesTest[1] = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAE=" + toBase64(polynomialInt[0].X)

		fmt.Println(sharesTest[0])
		fmt.Println(sharesTest[1])

		combined, err := Combine(sharesTest)
		if err != nil {
			fmt.Println("Fatal: combining: ", err)
		}
		fmt.Printf("The combined string: %x\n", combined)

		var sharesTest2 []string = make([]string, 2)
		sharesTest2[0] = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=" + toBase64(pubSum.Y)
		sharesTest2[1] = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAE=" + toBase64(polynomialInt[0].Y)

		fmt.Println(sharesTest2[0])
		fmt.Println(sharesTest2[1])

		combined2, err := Combine(sharesTest2)
		if err != nil {
			fmt.Println("Fatal: combining: ", err)
		}
		fmt.Printf("The combined string: %x\n", combined2)
		...
		A1 := new(ecdsa.PublicKey)
		A1.Curve =crypto.S256()
		A1.X, _ = big.NewInt(0).SetString(combined, 16)
		A1.Y, _ = big.NewInt(0).SetString(combined2, 16)

		verifier.GenerateSubAccount(A1, GetSAccountPub())
