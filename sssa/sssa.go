...
const (
	...
	//DefaultRandomStr = "340282366920938463463374607431768211455"
	//DefaultPrimeStr = "95792089237316195423570985008687907853269984665640564039457584007913129639747"
...
func CombineECDSAPubs(shares []string) (string, error) {
	...
	for j := range secret {
		...
		for i := range secrets { // LPI sum loop
			...
			/*
			working := big.NewInt(0).Set(origin_x)
			working = working.Mul(working, numerator)
			working = working.Mul(working, modInverse(denominator))
