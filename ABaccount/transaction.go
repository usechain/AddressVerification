...
func (tx *Transaction) IsMainAuthentication() bool {
	fmt.Println("IsMainAuthentication func entered!")
...
func (tx *Transaction) IsSubAuthentication() bool {
	fmt.Println("IsSubAuthentication func entered!")
	//The authentication tx payload must longer than 36 bytes
	//Added levelTag and address type
	if len(tx.Data()) <= 4 + 32 * 30 {
		return false
...
func (tx *Transaction) IsAuthentication() bool {
	fmt.Println("IsAuthentication func entered!")
	//The authentication tx payload must longer than 36 bytes
	//Added levelTag and address type
...
func (tx *Transaction) CheckCertificateSig(_from common.Address) error {
	...
	pubPosition := binary.BigEndian.Uint16(tx.Data()[34:36])
	//get second parameter start position
	signPosition := binary.BigEndian.Uint16(tx.Data()[66:68])
	//get third parameter start position
	caPosition :=  binary.BigEndian.Uint16(tx.Data()[98:100])

	//get pubkey length
	pubLen := binary.BigEndian.Uint16(tx.Data()[pubPosition + 34: (pubPosition + 36)])
	//get sign length
	signLen := binary.BigEndian.Uint16(tx.Data()[signPosition + 34: (signPosition + 36)])
	//get CA length
	caLen := binary.BigEndian.Uint16(tx.Data()[caPosition + 34 :(caPosition + 36)])

	pub := tx.Data()[pubPosition + 36 : pubPosition + 36 + pubLen]
	sign := tx.Data()[signPosition + 36 : signPosition + 36 + signLen]
	ca := tx.Data()[caPosition + 36 : caPosition + 36 + caLen]

	fmt.Printf("pubkey: %x\nsign: %x\nca: %x\n", pub, sign, ca)

	pubHex, _ := hexutil.Decode(string(pub))
	pubKey := crypto.ToECDSAPub(pubHex)
	if crypto.PubkeyToAddress(*pubKey) != _from {
		return errors.New("the pubkey & address doesn't match")
	}
	fmt.Println("public key check")

	fmt.Printf("The signature: %x\n", string(sign))
	fmt.Println("The signature:", string(sign))
	fmt.Println("The address:", _from.String())
	err := crypto.CheckUserCertStandard(string(ca), _from, sign)
	if  err != nil {
		return errors.New("the CA cert is illegal")
	}
	fmt.Println("CA cert check ok")
